...
#from copiaParticles import ParticleFilter
...
while not done:
    ...
    print pf.X
    for p in pf.X: 
        print "Antes Move ", p# e se passar o indice da particula?
        pf.move(p,u)
        print "Apos Move ", p
        for medida in z:
            pf.sense(p, medida)
    ...
    implementar random particles
    mudar modo de reamostragem
    escrever isso
    ...
    novoVetor = copy.deepcopy(pf.X)
    for elemento in range(len(pf.X)):
        print pf.X
        print "ELEMENTO DE pf.X"
        print elemento, pf.X[elemento], "COPIA ", novoVetor[elemento]
        print " ''' "
        print "Apos move"
        pf.move(elemento,u)
        print pf.X[elemento]
        ...
        for medida in z:
            ...
            pf.sense(elemento,medida)
        
        soma += pf.X[elemento].peso
        print " ''' "
        """
    soma2 = 0
    for p in pf.X:
